...
#define TFT_HT (TFT_WIDTH+TFT_HSYNC_BACK_PORCH+TFT_HSYNC_FRONT_PORCH)
#define TFT_HPS TFT_HSYNC_BACK_PORCH
#define TFT_VT (TFT_HEIGHT+TFT_VSYNC_BACK_PORCH+TFT_VSYNC_FRONT_PORCH)
#define TFT_VPS TFT_VSYNC_BACK_PORCH
...
#define Lcd_Write_Cmd(Cmd) *(uint16_t*) (LCD_REG) = Cmd
#define Lcd_Write_Data(data) *(uint16_t*) (LCD_DATA) = data
...
void TFT_Draw_H_Dot_Line(uint16_t x, uint16_t y, uint16_t length, uint16_t size,uint16_t color);
void TFT_Draw_V_Dot_Line(uint16_t x, uint16_t y, uint16_t length, uint16_t size,uint16_t color);
...
void TFT_Draw_BMP(uint16_t x, uint16_t y, uint16_t pwidth, uint16_t pheight, const uint16_t *p);
